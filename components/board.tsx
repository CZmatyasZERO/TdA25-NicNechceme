...
import OIcon from "./../public/icons/O/red.svg";

type Cell = "X" | "O" | "";

const TicTacToeBoard = (params: { board: Cell[][], onClick?: (x: number, y: number) => void, gap?:number, interative?: boolean, lines?: boolean, fullWidth?: boolean }) => {
            ...
            <div className={classes.board} style={{gap, width: params.fullWidth ? "100%" : undefined}}>
                {params.board.map((row, i) => (
                    <div key={i} style={{gap}} className={classes.row}>
                        {row.map((cell, j) => (
                            <div key={j} className={classes.cell}  onClick={() => cell === "" && params.onClick && params.interative && params.onClick(i, j)} style={params.lines ? {border: "1px solid black"} : {}}>
                                {cell === "X" ? (
