...
import { checkGameEnd, createEmptyBoard, findBestMove } from "./../lib/tictactoe"
...
export function GamePage({
    ...
    try {
      axios.post("/api/v1/games", data).then((response) => {
        if (response.status === 201) {
          ...
          router.replace("/game/" + response.data.uuid, undefined, {
            shallow: true,
          })
          setIsModified(false)
        }
  ...
  function editPuzzleSubmit(values: any) {
    ...
    try {
      axios.put("/api/v1/games/" + savedGame?.uuid, data).then((response) => {
  ...
  const [board, setBoard] = useState<Board>(savedGame.board)
  ...
  const [touchModeDoubleClick, setTouchModeDoubleClick] = useState({
    x: -1,
    y: -1,
  })
          ...
          <Flex gap={10} wrap="wrap" justify="center" style={{ width: "100%", maxWidth: "1000px" }}>
                  ...
                  <Center style={{ height: "100%", width: "100%" }}>
                    <Paper shadow="xs" p="md" radius="md">
                      <Stack gap={10} align="center">
                        <TdAIcon
                          type={gameResult.winner ? gameResult.winner : "Thinking"}
                          size={100}
                          color={gameResult.winner ? (gameResult.winner === "X" ? "blue" : "red") : "black"}
                        />
            ...
            <Paper shadow="xs" p="md" radius="md" style={{ flex: 1, minWidth: "230px", maxWidth: "350px" }}>
              <Tabs defaultValue={savedGame.uuid !== "" ? "puzzle" : "game"}>
                <Tabs.List>
                  <Tabs.Tab value="game">Hra</Tabs.Tab>
                  {savedGame.uuid !== "" && (
                    <Tabs.Tab value="puzzle" hidden={savedGame.uuid !== ""}>
                      Úloha
                    </Tabs.Tab>
                  )}

                  <Tabs.Tab value="settings">Nastavení</Tabs.Tab>
                </Tabs.List>
                ...
                <Tabs.Panel value="game">
                  <Stack gap={10}>
                    ...
                    <Group>
                      <Text>
                        <b>Na řadě jsou:</b> {onTurn === "X" ? "Křížky" : "Kolečka"}
                      </Text>
                    ...
                    <form onSubmit={createPuzzleForm.onSubmit((values) => createPuzzleSubmit(values))}>
                      ...
                      <Group justify="flex-end" mt="md">
                        <Button type="submit" disabled={!isModified || gameResult.finished}>
                          Uložit
                        </Button>
                      </Group>
                    </form>
                ...
                <Tabs.Panel value="settings">
                  <Stack gap={10}>
                    ...
                    <Checkbox
                      label="Režim pro dotykové obrazovky"
                      checked={touchMode}
                      onChange={(event) => setTouchMode(event.currentTarget.checked)}
                    />
                    <Checkbox
                      label="Režim pro projektor"
                      checked={projectorMode}
                      onChange={(event) => setProjectorMode(event.currentTarget.checked)}
                    />
                ...
                <Tabs.Panel value="puzzle">
                  {savedGame.uuid !== "" && (
                    <Stack gap={10}>
                      ...
                      <Group>
                        <Text>
                          <b>Název:</b> {savedGame.name}
                        </Text>
                      </Group>
                      <Group>
                        <Text>
                          <b>Obtížnost:</b> {TdADifficulty[savedGame.difficulty]}
                        </Text>
                        <TdAIcon size={30} type={savedGame.difficulty} color="blue" />
                      </Group>
                      <Group>
                        <Text>
                          <b>Kolo úlohy:</b> {Math.floor(savedGame.board.reduce((acc, row) => acc + row.filter((cell) => cell !== "").length, 0) / 2)}
                        </Text>
                      </Group>
                      ...
                      <form onSubmit={editPuzzleForm.onSubmit((values) => editPuzzleSubmit(values))}>
                        ...
                        <Group justify="flex-end" mt="md">
                          <Button type="submit" disabled={gameResult.finished}>
                            Upravit
                          </Button>
                        </Group>
                      </form>
                      <Button color="red" onClick={deletePuzzle}>
                        Smazat úlohu
                      </Button>
                    </Stack>
                  )}
